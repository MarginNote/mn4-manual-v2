...
用法（在仓库根，建议用 .venv）：
  doit              全量/增量构建（page → media → config → site）
  doit -n 2         并行（媒体编码受益最大）
  doit clean        删除所有产物
  doit list         列出任务组
  doit page         只构建正文；  doit "page:<id>" 单页
  doit media        只编码媒体
  doit config       toc.yaml → mkdocs.yml + 首页占位
...
from ssg import task_config, task_site  # noqa: F401

DOIT_CONFIG = {"default_tasks": ["page", "media", "config", "site"], "verbosity": 1}
